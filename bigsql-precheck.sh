...
check_disk_space()
{
   ...
   # Check space on /var
   if [[ -d  ${TEMP_DIR} ]]; then
      cmd5="`df -P ${TEMP_DIR} | grep -vE '^Filesystem|tmpfs|cdrom' | awk '{ print $4 }'`"
...
check_all_clients()
{
   ...
   timeout 300 su - ${BIGSQL_USER} sh -c "hcat -h" >> ${SCRIPT_LOG} 2>&1
   cmd_rc=$?
...
Install all the required clients and restart the installation."
...
Fix the required clients and restart the installation. If the environment is kerberized, \
...
check_user_shell()
{
   ...
   if [ ! -z "${HBASE_USER}" ]; then
      ...
      if [ $cmd_rc -ne 0 ]; then
         ...
         user_action_msg="The hbase user's shell is $shell_used which is not valid. Please set the shell to a valid login shell such as bash or ksh."
         rc=1
      fi
...
################################################################################
# 29 - Ensure that (rhel7) rhel-x86_64-server-optional-7 is installed
################################################################################
check_rhel7_ha_prereqs()
{
   ...
   log "${FUNCNAME}" "Enter - Check that rhel 7 optional packages for is installed"

   rhel7_optional_pkg_1="rhel-x86_64-server-optional-7"
   rhel7_optional_pkg_2="rhel-7-server-optional-rpms"
   rhel7_optional_pkg_logname="rhel 7 optional"

   user_action_msg="The package ${rhel7_optional_pkg_1} or ${rhel7_optional_pkg_2} is missing and needs to be installed."
   ...
   if [ -f /etc/redhat-release ]; then
      ...
      if [ ${PLATFORM} == "x86_64" ]; then
         ...
         if [ $? -eq 0 ]; then
            log "${FUNCNAME}" "Enter - Check that ${rhel7_optional_pkg_1} or ${rhel7_optional_pkg_2} is installed"

            log "${FUNCNAME}" "Get uname -a"
            uname -a | grep "x86_64" >> ${SCRIPT_LOG} 2>&1

            # Check that optional is installed
            log "${FUNCNAME}" "Looking in yum repolist for ${rhel7_optional_pkg_1} or ${rhel7_optional_pkg_2} "

            yum list | egrep "${rhel7_optional_pkg_1}|${rhel7_optional_pkg_2}"  >> ${SCRIPT_LOG} 2>&1
            user_rc=$?

            if [[ ${user_rc} -ne 0 ]]; then
               log "${FUNCNAME}" "The package ${rhel7_optional_pkg_1} or ${rhel7_optional_pkg_2} is missing and needs to be installed."
               rc=${BIGSQL_PRECHECK_WARN_CODE}
            else
               rc=0
            fi
   ...
   # Log to the report log
   report_check $rc "Check ${rhel7_optional_pkg_logname} installation" "${user_action_msg}"

   log "${FUNCNAME}" "Exit - Check that ${rhel7_optional_pkg_logname} is installed with rc=$rc"
...
check_env_files()
{
   ...
   # Log to the report log
   report_check $rc "Check ownership of bigsql env files" "${user_action_msg}"
...
echo "======================================================================" >> ${SCRIPT_LOG}
echo "bigsql-precheck executed with:" >> ${SCRIPT_LOG}
...
if [ -f ${SSH_HOSTLIST_FILE} ]; then
   ...
   do
      echo "$index ${THIS_HOST_LIST_ARRAY[index]}"
...
if [ ${ALL_HOSTS} -eq 1 ]; then
  ...
  if [ -f ${SSH_HOSTLIST_FILE} ]; then
    ...
    echo "${SSH_HOSTLIST_FILE} does not exist, please create a file with list of hosts to run prechecker on"
    echo "host1.mydomain.com"
    echo "nost2.mydomain.com"
    exit 1
