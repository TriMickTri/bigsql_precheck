...
STANDBY_CHECK=0
EXTERNALLY_MANAGED_SSH_KEYS="True"
EXTERNALLY_MANAGED_USERS="True"
...
# 44 - Validate bigsql admin group name length
# 45 - Validate bigsql admin group name for lowercase characters
# 46 - Check passwordless ssh for bigsql user
# 47 - Check bigsql id exists
# 48 - Check bigsql group exists
...
check_disk_space()
{
   ...
   if [[ -d ${TEMP_DIR} ]]; then
      ...
      df -P ${TEMP_DIR} | grep '^tmpfs' > /dev/null 2>&1
      if [ $rc -eq 0 ]; then
...
check_sudoers()
{
   ...
   # Log to the report log
   if [ $result -ne 1 ]; then
       found_tty_entry=${BIGSQL_PRECHECK_ERR_CODE}
...
check_all_clients()
{
   ...
   timeout 60 su - ${SQOOP_USER} sh -c "sqoop version" >> ${SCRIPT_LOG} 2>&1
   cmd_rc=$?
   ...
   timeout 60 su - ${HCAT_USER} sh -c "hcat -h" >> ${SCRIPT_LOG} 2>&1
   cmd_rc=$?
...
check_non_root_cron()
{
   ...
   if [[ ${HAVE_NON_ROOT_ADM_USER} -eq 1 ]]; then
      ...
      $(${SUDO_CMD} su - ${SUDO_SSH_USER} sh -c "crontab -l > ${TMP_FILE} 2>&1")

      # See if there is a crontab; this is OK
      if grep -q "no crontab for" ${TMP_FILE}; then
         log "${FUNCNAME}" "There is no crontab and user ${SUDO_SSH_USER} can use crontab."
      # Check is user not allowed to run cron
      elif grep -q "not allowed" ${TMP_FILE}; then
         log "${FUNCNAME}" "${SUDO_SSH_USER} does not have permission to use crontab."

         user_action_msg="The non-root admin user \"${SUDO_SSH_USER}\" is not allowed to use crontab. Use of crontab is needed to run Big SQL metrics. Change the cron configuration to allow cron usage for the user."
         rc=${BIGSQL_PRECHECK_ERR_CODE}
      # if not the above 2, there is already a cron job
      else
         log "${FUNCNAME}" "${SUDO_SSH_USER} already has a cron job:"
...
################################################################################
# 44 - Check the length of bigsql admin group name
################################################################################
validate_bigsql_admin_group_name()
{
   logLevel=$((logLevel+1))
   local rc=0
   local user_action_msg=""

   log "${FUNCNAME}" "Enter - Check bigsql group name length"

   # check length
   user_name_length=${#BIGSQL_ADMIN_GROUP_NAME}

   if [[ $user_name_length -gt 32 ]]; then
      user_action_msg="The name \"${BIGSQL_ADMIN_GROUP_NAME}\" used for the Big SQL Admin group name is too long. It needs to be 32 characters or less."
      rc=${BIGSQL_PRECHECK_ERR_CODE}
   fi

   # Log to the report log
   report_check $rc "Big SQL Admin group name length" "${user_action_msg}"

   log "${FUNCNAME}" "Exit - Check bigsql group name length rc=$rc"
...
################################################################################
# 45 - Validate the bigsql admin group name to lowercase string.
################################################################################

validate_bigsql_admin_name_is_lowercase()
{
   logLevel=$((logLevel+1))
   local rc=0
   local user_action_msg=""
   ...
   log "${FUNCNAME}" "Enter - Check bigsql admin group name for lowercase letters."

   log "${FUNCNAME}" "Testing bigsql admin group name "

   # Check that input is uppercase
   if [ -z `echo ${BIGSQL_ADMIN_GROUP_NAME} | tr -d "[:upper:]"` ]; then
	  log "${FUNCNAME}" "BigSQL Admin group name is uppercase in nature."
	  user_rc=1
   fi

   if [[ ${user_rc} -ne 0 ]]; then
      log "${FUNCNAME}" "The BigSQL admin group name string is non-standard. "
      user_action_msg="The BigSQL admin group name contains uppercase characters. Please ensure that the value of group name contains only lowercase. "
      rc=${BIGSQL_PRECHECK_ERR_CODE}
   fi

   # Log to the report log
   report_check $rc "Lowercase BigSQL admin group name check" "${user_action_msg}"

   log "${FUNCNAME}" "Exit - Lowercase BigSQL admin group name check with rc=$rc"
...
check_passwordless_ssh_bigsql()
{
   ...
   log "${FUNCNAME}" "Enter - Check passwordless ssh setup for Bigsql User"
   ...
   if [[ ${HAVE_HOSTLIST_FILE} -eq 1 ]]; then
      ...
      for NODE in ${NODE_LIST[@]}; do
            ...
            do
               ...
               # If there's a retry, this entry will appear multiple times
               log "${FUNCNAME}" "Checking ssh connection from this node to node ${NODE}"

               ${SUDO_CMD} su - ${BIGSQL_USER} sh -c "${SSH_BATCH} ${NODE} sh -c "exit 0"" > /dev/null 2>&1
   ...
   # If any of the tests failed, return a failure for the check
   if [[ $head_test_rc -ne 0 ]]; then
      rc=${BIGSQL_PRECHECK_ERR_CODE}
   fi

   # Detailed error explanation in the report
   if [[ $rc -ne 0 ]]; then
      user_action_msg="Passwordless ssh needs to be configured between this host and other nodes."
   fi

   # Log to the report log
   report_check $rc "Passwordless ssh test for bigsql user" "${user_action_msg}"

   log "${FUNCNAME}" "Exit - Check passwordless ssh setup for Bigsql User with rc=$rc"
...
# 47 - Check whether Bigsql User exists
check_bigsql_user_exists()
{
   logLevel=$((logLevel+1))
   local rc=0
   local user_action_msg=""

   log "${FUNCNAME}" "Enter - Check whether BigSQL User exists"

   #Fetch bigsql user
   id $BIGSQL_USER > /dev/null 2>&1

   if [[ $? -ne 0 ]]; then
      user_action_msg="The user \"${BIGSQL_USER}\" does not exist. Please create the user before proceeding with install"
      rc=${BIGSQL_PRECHECK_ERR_CODE}
   fi

   # Log to the report log
   report_check $rc "Check whether BigSQL User exists" "${user_action_msg}"

   log "${FUNCNAME}" "Exit - Check whether BigSQL User exists with rc=$rc"
...
# 48 - Check whether Bigsql Group exists
check_bigsql_group_exists()
{
   logLevel=$((logLevel+1))
   local rc=0
   local user_action_msg=""

   log "${FUNCNAME}" "Enter - Check whether BigSQL Group exists"

   #Fetch bigsql admin group
   if [ $(getent group $BIGSQL_ADMIN_GROUP_NAME) ]; then
      user_action_msg="The group \"${BIGSQL_ADMIN_GROUP_NAME}\" exists"
   else
      user_action_msg="The group \"${BIGSQL_ADMIN_GROUP_NAME}\" does not exist. Please create the group before proceeding with install"
      rc=${BIGSQL_PRECHECK_ERR_CODE}
   fi

   group_id=$(cut -d: -f3 < <(getent group $BIGSQL_ADMIN_GROUP_NAME))
   if [[ $group_id != $BIGSQL_ADMIN_GROUP_ID ]]; then
      user_action_msg="The group \"${BIGSQL_ADMIN_GROUP_NAME}\" does not have correct GID. "
      rc=${BIGSQL_PRECHECK_ERR_CODE}
   fi

   # Log to the report log
   report_check $rc "Check whether BigSQL Group exists" "${user_action_msg}"

   log "${FUNCNAME}" "Exit - Check whether BigSQL Group exists with rc=$rc"
...
################################################################################
# Build the command options for the remote invocation
################################################################################
# The remote command does not need all the paramters passed in.
# Remove:
# - '-A' for all hosts
# - '-P' for parallel exec
# - '-l' for host file list since it's not needed on remote hosts
# 
################################################################################
build_remote_options()
{
   b_OPT=""; [ "${HBASE_USER}"            == "" ] || b_OPT=" -b ${HBASE_USER}"
   B_OPT=""; [ "${BIGSQL_FIRST_HEAD}"     == "" ] || B_OPT=" -B ${BIGSQL_FIRST_HEAD}"
   d_OPT=""; [ "${BIGSQL_DATA_DIRS}"      == "" ] || d_OPT=" -d ${BIGSQL_DATA_DIRS}"
   f_OPT=""; [ "${BIGSQL_PORT}"           == "" ] || f_OPT=" -f ${BIGSQL_PORT}" 
   F_OPT=""; [ "${FORCE_ERR}"             -ne 1 ] || F_OPT=" -F"
   H_OPT=""; [ "${BIGSQL_HOME}"           == "" ] || H_OPT=" -H ${BIGSQL_HOME}"
   i_OPT=""; [ "${HIVE_USER}"             == "" ] || i_OPT=" -i ${HIVE_USER}"
   I_OPT=""; [ "${HIVE_METASTORE_HOST_2}" == "" ] || I_OPT=" -I ${HIVE_METASTORE_HOST_2}"
   L_OPT=""; [ "${SCRIPT_LOG_DIR}"        == "" ] || L_OPT=" -L ${SCRIPT_LOG_DIR}"
   m_OPT=""; [ "${HIVE_METASTORE_HOST_1}" == "" ] || m_OPT=" -m ${HIVE_METASTORE_HOST_1}"
   M_OPT=""; [ "${MODE}"                  == "" ] || M_OPT=" -M ${MODE}"
   n_OPT=""; [ "${TARGET_NODE}"           == "" ] || n_OPT=" -n ${TARGET_NODE}"
   N_OPT=""; [ "${NAMENODE}"              == "" ] || N_OPT=" -N ${NAMENODE}"
   O_OPT=""; [ "${CONFIGURED_HOSTNAME}"   == "" ] || O_OPT=" -O ${CONFIGURED_HOSTNAME}"
   p_OPT=""; [ "${DB2_PORT}"              == "" ] || p_OPT=" -p ${DB2_PORT}"
   R_OPT=""; [ "${HDFS_PRINCIPAL}"        == "" ] || R_OPT=" -R ${HDFS_PRINCIPAL}"
   s_OPT=""; [ "${SECURITY_TYPE}"         == "" ] || s_OPT=" -s ${SECURITY_TYPE}"
   S_OPT=""; [ "${STANDBY_CHECK}"         -ne 1 ] || S_OPT=" -S"
   t_OPT=""; [ "${TIMESTAMP}"             == "" ] || t_OPT=" -t ${TIMESTAMP}"
   T_OPT=""; [ "${HDFS_KEYTAB}"           == "" ] || T_OPT=" -T ${HDFS_KEYTAB}"
   u_OPT=""; [ "${BIGSQL_USER}"           == "" ] || u_OPT=" -u ${BIGSQL_USER}"
   v_OPT=""; [ "${BIGSQL_VAR}"            == "" ] || v_OPT=" -v ${BIGSQL_VAR}"
   V_OPT=""; [ "${VERBOSE}"               -ne 1 ] || V_OPT=" -V"
   x_OPT=""; [ "${HDFS_USER}"             == "" ] || x_OPT=" -x ${HDFS_USER}"
   z_OPT=""; [ "${BIGSQL_UID_CHECK}"      == "" ] || z_OPT=" -z ${BIGSQL_UID_CHECK}"
   Z_OPT=""; [ "${SUDO_SSH_USER}"         == "" ] || Z_OPT=" -Z ${SUDO_SSH_USER}"
   
   REMOTE_CMD_OPTIONS="${b_OPT}${B_OPT}${d_OPT}${f_OPT}${F_OPT}${H_OPT}${i_OPT}${I_OPT}${L_OPT}"
   REMOTE_CMD_OPTIONS="${REMOTE_CMD_OPTIONS}${m_OPT}${M_OPT}${n_OPT}${N_OPT}${O_OPT}${p_OPT}${R_OPT}"
   REMOTE_CMD_OPTIONS="${REMOTE_CMD_OPTIONS}${S_OPT}${s_OPT}${t_OPT}${T_OPT}${u_OPT}${v_OPT}${V_OPT}"
   REMOTE_CMD_OPTIONS="${REMOTE_CMD_OPTIONS}${z_OPT}${x_OPT}${Z_OPT}"
}
...
pre_add_host_checks()
{
   ...
   if [[ $SKIP_PRECHECKING -eq 0 ]]; then
      ...
      # Validate bigsql admin group name for length
      validate_bigsql_admin_group_name

      # Validate bigsql admin group name for lowercase string.
      validate_bigsql_admin_name_is_lowercase
      ...
      if [[ "${EXTERNALLY_MANAGED_SSH_KEYS}" == "False" ]]; then
          # Check passwordless SSH for bigsql user
          check_passwordless_ssh_bigsql
      fi

      if [[ "${EXTERNALLY_MANAGED_USERS}" == "False" ]]; then
          # Check bigsql user & group exists
          check_bigsql_user_exists
          check_bigsql_group_exists
      fi
...
Usage: bigsql-precheck.sh [options]
   ...
   Options:
      ...
      -a bigsql admin group name    BigSQL Admin group name
      -b hbase user     Username for hbase
      -g bigsql group id    BigSQL Admin group id
      ...
      -K externally managed ssh keys Externally managed SSH Keys
      -X externally managed users Externally managed Users
      -h                This help

      Example of a command to run before an install:

./bigsql-precheck.sh -M PRE_ADD_HOST -a bigsqladm -g 43210 -u bigsql -l /tmp/list_of_hosts.txt -z bigsql,2824,hadoop,515 -s os -x hdfs -b hbase -i hive -p 32051 -f 28051 -K -X -V -F
...
show_vars()
{
    ...
    BIGSQL_UID_CHECK         : ${BIGSQL_UID_CHECK}
    BIGSQL_ADMIN_GROUP_NAME  : ${BIGSQL_ADMIN_GROUP_NAME}
    BIGSQL_ADMIN_GROUP_ID    : ${BIGSQL_ADMIN_GROUP_ID}
    HDFS_USER                : ${HDFS_USER}
    ...
    EXTERNALLY_MANAGED_SSH_KEYS : ${EXTERNALLY_MANAGED_SSH_KEYS}
    EXTERNALLY_MANAGED_USERS : ${EXTERNALLY_MANAGED_USERS}
...
################################################################################
while getopts "a:b:B:d:f:g:H:i:I:l:L:m:M:n:N:O:p:R:s:t:T:u:v:z:x:KXAFhPSVZ:" o; do
    case "${o}" in
        A) ALL_HOSTS=1
           ;;
        a) BIGSQL_ADMIN_GROUP_NAME="${OPTARG}"
           ;;
        b) HBASE_USER="${OPTARG}"
           ;;
        ...
        f) BIGSQL_PORT="${OPTARG}"
           ;;
        g) BIGSQL_ADMIN_GROUP_ID="${OPTARG}"
           ;;
        F) FORCE_ERR=1
        ...
        v) BIGSQL_VAR="${OPTARG}"
           ;;
        K) EXTERNALLY_MANAGED_SSH_KEYS="False"
           ;;
        X) EXTERNALLY_MANAGED_USERS="False"
           ;;
        V) VERBOSE=1
           ;;
...
if [[ -z ${SCRIPT_LOG_DIR+x} ]]; then
   LOGGING_DIR=${TMP}/${BIGSQL_USER}/logs
...
# Derive the non-root ambari user if possible
NON_ROOT_ADM_USER=$(ps -ef | grep AmbariAgent | grep -v grep | awk '{print $1}') > /dev/null 2>&1

if [[ ${NON_ROOT_ADM_USER} != "root" ]]; then
  ...
  if [ -f ${SSH_HOSTLIST_FILE} ]; then
    ...
    if [ ${PARALLEL} -eq 0 ]; then
      ...
      do
        ...
        # remove some of the originaloptions
        build_remote_options
